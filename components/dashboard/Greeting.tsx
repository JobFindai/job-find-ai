    ...
    <div className=" flex flex-col lg:gap-10 gap-5">
      ...
      <div className="grid lg:grid-cols-5 grid-cols-2 grid-rows-2 lg:grid-rows-1 min-h-3  gap-4">
        <DashBoardStatCard
          ...
          count={stats?.totalApplied || 0}
        />
        <DashBoardStatCard title="In Review" count={stats?.inReview || 0} />
        <DashBoardStatCard title="Interviews" count={stats?.interviews || 0} />
        <FitScore score={stats?.averageFitScore || 0} />
      </div>
    </div>
  );
}
...
function DashBoardStatCard({ title, count }: { title: string; count: number }) {
  return (
    <div className="bg-white px-4 lg:py-7 py-5 rounded-md flex lg:justify-between justify-evenly flex-col lg:shadow-lg/5">
    ...
    <>
      {isMobile ? (
        <div className="bg-white flex flex-col justify-center px-4 gap-1">
          ...
          <span className="lg:text-3xl text-4xl font-bold">{score}%</span>
        ...
        <div className="lg:col-span-2  bg-white rounded-md flex flex-col gap-3 px-4 py-5">
          ...
          <div className="flex flex-col gap-0.5  flex-1">
            <span className="text-base font-medium">
              Your average fitscore: <b>{score}%</b>
            </span>
